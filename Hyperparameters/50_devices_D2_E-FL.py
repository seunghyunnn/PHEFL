...
def lambda_handler(result):
    result_file_name = '50_devices_D2_E-FL.txt'
...
while accuracy < 0.99: 
    ...
    for i in range(NUM_OF_DEVICE):
        temp = build_model(1)
        temp.set_weights(edge_models[i//NUM_OF_DEVICE_PER_EDGE].get_weights())
        device_models.append(temp)
    ...
    for i in range(NUM_OF_EDGE): 
        ...
        for j in range(len(device_models[0].get_weights())): # for every weights 
            temp = 0 
            for k in range(NUM_OF_DEVICE_PER_EDGE) : # 0 - 9, k-th device in i-th edge
                temp = temp + device_models[i*NUM_OF_DEVICE_PER_EDGE + k].get_weights()[j] * (len(edge_X_train_dataset[str(i)][k])/edge_data_size[str(i)])
            edge_weights.append(temp)
        edge_models_weights.append(edge_weights)
    edge_models = []
