...
def lambda_handler(result):
    result_file_name = 'D2_Aggr_freq_5_PHE-FL.txt'
...
for i in range(NUM_OF_EDGE):
    temp = build_model(1)
    temp.set_weights(edge_models[i].get_weights())
    new_edge_models.append(temp)
print(len(new_edge_models), type(new_edge_models[0]))
...
for i in range(len(sum_edge_performance)):
    edge_accuracies += f"edge-{i} : {sum_edge_performance[i]} | {sorted(sum_edge_misclassified[i].items(), key=lambda item: item[1], reverse=True)}\n"
edge_accuracies += f"edge average: {sum(sum_edge_performance)/ 10}"
buffer += str(iteration_count) + "th: " + "not-personalized : \n"+"each edges model: \n" + str(edge_accuracies) + "\n---------------\n\n"
...
while accuracy < 0.99: 
    ...
    #loss, accuracy = cloud_models[10].evaluate(X_test_full, y_test,verbose=0)
    #misclassified= get_incorrect_label_counts(cloud_models[10],X_test_full,y_test)
    #buffer += str(iteration_count) + "th: " + "cloud model against entire testset: " + str(accuracy) +"\n"
    #buffer += str(iteration_count) + "th: " + f"cloud model against entire testset misclassified: {misclassified}\n"
    ...
    if iteration_count % AGGREGATION_FREQ == 4: 
        print(f"**********************   ROUND {iteration_count} Personalization started!    **************") 
        ############################ PERSONALIZATION ############################
